...
export default function CASubAdmin() {
    ...
    const [activeTab, setActiveTab] = useState('sub-admin'); // sub-admin | permissions
    const [form, setForm] = useState({ name: '', email: '', password: 'subadmin123' });
    ...
    const handleAdd = () => {
        ...
        setShowAdd(false);
        setForm({ name: '', email: '', password: 'subadmin123' });
    };
    ...
    return (
        <div className="page-enter">
            <PageHeader title="Sub Admin" subtitle={`${companyUsers.length} team members`}>
                    ...
                    {filtered.length === 0 ? (
                        <EmptyState icon={Users} title="No Human Resources" description="Add your first team member to get started."
                            action={<button className="btn-primary"
                                ...
                                <Plus size={14} /> Add Sub Admin</button>} />
                    ) : (
