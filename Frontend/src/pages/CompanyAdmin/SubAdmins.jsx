...
const ALL_ROLES = [
    ...
    { value: 'executive', label: 'Executive', color: '#6366F1' },
];
...
export default function CASubAdmins() {
    const {
    ...
    const [form, setForm] = useState({ name: '', email: '', password: 'sa123' });
    ...
    const handleAdd = () => {
        ...
        setShowAdd(false);
        setForm({ name: '', email: '', password: 'sa123' });
    };

    const handleEdit = () => {
        updateUser(editTarget.id, { name: form.name, email: form.email });
        setEditTarget(null);
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {!editTarget && (
                <FormGroup label="Temporary Password">
                    <input className="input-field" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="hr123" />
                </FormGroup>
            )}
            {/* Role preview */}
            {/* {form.role && (
    ...
    return (
        <div className="page-enter">
            <PageHeader title="Sub Admins" subtitle={`${companyUsers.length} team members`}>
            ...
            {/* <div style={{ display: 'flex', gap: 4, marginBottom: 18, padding: 4, borderRadius: 12, width: 'fit-content', background: 'var(--bg-secondary)', border: '1px solid var(--border-primary)' }}>
                {[{ id: 'employees', label: 'Employees' }, { id: 'permissions', label: 'Role Permissions' }].map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        style={{
                            padding: '6px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s',
            ...
            {activeTab === 'sub-admins' && (
                <>
                    ...
                    {filtered.length === 0 ? (
                        <EmptyState icon={Users} title="No Sub Admin Found" description="Add your first team member to get started."
                            action={<button className="btn-primary" onClick={() => setShowAdd(true)}><Plus size={14} /> Add Sub Admin</button>} />
                    ) : (
                                ...
                                return (
                                    <div key={u.id} className="card card-hover" style={{ padding: 16, position: 'relative' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                                            <span style={{
                                                fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 20, textTransform: 'capitalize',
            ...
            <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Team Member">
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20 }}>
                    ...
                    <button className="btn-primary" onClick={handleAdd}>Add Sub Admin</button>
                </div>
            </Modal>
            ...
            <ConfirmDialog
                ...
                title="Remove Sub Admin" message={`Remove "${deleteTarget?.name}" from the team? This action cannot be undone.`}
            />
        </div>
    );
}
