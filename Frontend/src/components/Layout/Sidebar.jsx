  ...
  const roleLabel =
    role === 'super_admin'
      ...
      : role === 'company_admin'
        ? 'Company Admin'
        : currentUser?.role
          ? (ROLE_LABELS[currentUser.role] || currentUser.role.replace('_', ' '))
          : 'User';
  ...
  return (
    ...
    >
      <div
        ...
        style={{ borderBottom: '1px solid var(--border-primary)', minHeight: 64 }}
      >
        <div
        ...
        {open && (
          <div>
            <div
              className="font-display font-700 tracking-widest text-base"
            ...
            <div
              ...
              style={{ color: accentColor, marginTop: 1, letterSpacing: '0.04em' }}
            >
              {roleLabel}
            </div>
          </div>
      ...
      <nav className="flex-1 py-4 px-2 overflow-y-auto">
        {role === 'user' ? (
            ...
            {USER_GROUPS.map((group) => {
              ...
              return (
                <div key={group.header} className="mb-4">
                  {open && (
                    <p
                      className="px-3 mb-1.5 text-xs font-700 tracking-widest uppercase"
          ...
          <>
            {open && (
              <p
                className="px-3 mb-2 text-xs font-700 tracking-widest uppercase"
                style={{ color: 'var(--text-muted)', fontSize: 10 }}
              >
                Navigation
              </p>
        ...
        {open && role === 'company_admin' && currentCompany && (
          ...
          >
            <p className="text-xs font-700 mb-0.5" style={{ color: '#0EA5E9' }}>
              Company
            </p>
        ...
        {open && role === 'user' && currentCompany && (
          ...
          >
            <p className="text-xs font-700 mb-0.5" style={{ color: accentColor }}>
              Your Company
            </p>
            <p className="text-sm font-700" style={{ color: 'var(--text-primary)' }}>
              {currentCompany.name}
        ...
        {currentUser && (
          <div className="flex items-center gap-2.5 p-3">
            <button
              ...
              title={role === 'user' ? 'View my profile' : currentUser.fullName}
              onMouseEnter={(e) => {
                if (role === 'user') e.currentTarget.style.transform = 'scale(1.05)';
              }}
              onMouseLeave={(e) => {
            ...
            {open && (
              <>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-700 truncate" style={{ color: 'var(--text-primary)' }}>
                    {currentUser.fullName}
                  </p>
                  <p
                    style={{
