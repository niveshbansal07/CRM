      ...
      ?.filter(
        (l) =>
          ...
          (
            ...
            l.contact?.toLowerCase().includes(low) ||
            l.email?.toLowerCase().includes(low)
          )
      )
      .slice(0, 3)
      .forEach((l) =>
        results.push({
          ...
          color: '#0EA5E9',
          icon: TrendingUp,
          path: role === 'company_admin' ? '/company/leads' : '/user/leads',
        })
      );

    deals
      ?.filter((d) => d.companyId === companyId && d.name?.toLowerCase().includes(low))
      .slice(0, 2)
      .forEach((d) =>
        results.push({
          type: 'Deal',
          label: d.name,
          sub: `₹${(d.value || 0).toLocaleString()}`,
          color: '#10B981',
          icon: Layers,
          path: role === 'company_admin' ? '/company/deals' : '/user/deals',
        })
      );
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="relative hidden md:block" ref={searchRef}>
          ...
          {showSearch && (
            <div
              className="absolute left-0 top-full mt-2 rounded-xl shadow-2xl z-50 overflow-hidden"
      ...
      <div className="flex items-center gap-3">
        <div
          className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-700"
          ...
          {showNotifs && (
            ...
            >
              ...
              >
                ...
                {unreadCount > 0 && (
                  <button onClick={markAllRead} className="text-xs font-600" style={{ color: '#0EA5E9' }}>
                    Mark all read
                  </button>
                )}
              </div>
                  ...
                  notifs.map((n) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseLeave={(e) => {
                          e.currentTarget.style.background = n.read ? 'transparent' : 'rgba(14,165,233,0.04)';
                        }}
                      >
                        <div
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs mt-0.5 font-600" style={{ color: n.color }}>
                            {n.time}
                          </p>
                        </div>
                        <div className="flex items-start gap-1 flex-shrink-0">
                          {!n.read && <div className="w-1.5 h-1.5 rounded-full mt-1.5" style={{ background: '#0EA5E9' }}></div>}
                          <button onClick={() => dismissNotif(n.id)} className="p-0.5 rounded transition-colors" style={{ color: 'var(--text-muted)' }}>
                            <X size={12} />
                          </button>
                        </div>
                      </div>
        ...
        <div className="relative" ref={userRef}>
          ...
          >
            ...
            <div className="hidden md:block text-left">
              <p className="text-sm font-600" style={{ color: 'var(--text-primary)', lineHeight: 1.2 }}>
                {currentUser?.fullName}
              </p>
              {currentCompany && (
          ...
          {showUserMenu && (
            ...
            >
              <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--border-primary)' }}>
                ...
                <div
                  className="mt-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-700"
                  style={{ background: roleStyle.bg, color: roleStyle.text }}
                >
                  {roleStyle.label}
                </div>
